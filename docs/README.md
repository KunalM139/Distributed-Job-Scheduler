...
## Future Improvements
- **Redis Caching:** Introduce Redis for caching high-traffic endpoints (e.g., dashboard stats).
- **Worker Auto-Scaling:** Deploy workers inside Kubernetes pods with HPA based on queue depth.
- **GraphQL API:** Provide a GraphQL layer for more flexible dashboard querying.
- **Email/Slack Notifications:** Alert project owners immediately when jobs hit the Dead Letter Queue.
